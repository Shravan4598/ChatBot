...
class RAGService:
    ...
    def build(self, source: str | Path) -> dict:
        ...
        try:
            ...
            loader = LoaderFactory.get_loader(str(source))
            documents = loader.load(source)

            chunks = self.splitter.split_documents(documents)

            vector_store = self.vector_service.create_vectorstore(chunks)

            self.retriever_service = RetrieverService(vector_store=vector_store)

            self.chain = RAGChain(self.retriever_service.get_retriever())
    ...
    # Stream
    # ==========================================================
    def stream(self, question: str, history: Optional[list[BaseMessage]] = None):
        try:
            if not self.is_initialized:
                raise RuntimeError("No document has been indexed.")
            yield from self.chain.stream(question=question, history=history)
        except Exception as e:
            raise ChatBotException(e)
    ...
    def retrieve_documents(self, query: str):
        if self.retriever_service is None:
            return []
        return self.retriever_service.retrieve(query)
    ...
    def retrieve_context(self, query: str) -> str:
        if self.retriever_service is None:
            return ""
        return self.retriever_service.retrieve_context(query)

    # ==========================================================
    # Metadata
    # ==========================================================
    def retrieve_metadata(self, query: str):
        if self.retriever_service is None:
            return []
        return self.retriever_service.retrieve_metadata(query)
